...
# Pour Raspberry Pi 4 avec IP publique 37.66.21.17
...
test_server() {
    ...
    # Tester la connexion
    if curl -s -f "http://37.66.21.17:$PORT/api/health" >/dev/null; then
        echo "✅ Serveur fonctionne correctement"
        echo "🌐 URL: http://37.66.21.17:$PORT"
        echo "🔗 WebSocket: ws://37.66.21.17:$PORT"
    else
...
show_final_info() {
    ...
    echo "   • API: http://37.66.21.17:$PORT/api"
    echo "   • Santé: http://37.66.21.17:$PORT/api/health"
    echo "   • Polygones: http://37.66.21.17:$PORT/api/polygons"
    echo "   • Messages: http://37.66.21.17:$PORT/api/messages"
    echo "   • WebSocket: ws://37.66.21.17:$PORT"
    echo ""
    ...
    echo "   1. Modifier votre app Android pour utiliser http://37.66.21.17:$PORT"
    echo "   2. Tester la connexion avec l'app"
    echo "   3. Migrer les données existantes"
    echo ""
    echo "🆘 En cas de problème:"
    echo "   • Vérifiez les logs: pm2 logs"
    echo "   • Testez la connexion: curl http://37.66.21.17:$PORT/api/health"
    echo "   • Redémarrez le serveur: pm2 restart $SERVICE_NAME"
    echo ""
}
...
# Gestion des erreurs
set -e
trap 'echo "❌ Erreur durant l'\''installation à la ligne $LINENO"' ERR
